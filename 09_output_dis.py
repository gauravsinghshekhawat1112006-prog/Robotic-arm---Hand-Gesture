...
import threading
# ================== WEBSOCKET CONFIG ==================
# WEBSOCKET_URL = "ws://192.168.1.XX:81"   # <-- ADD LATER
SEND_INTERVAL = 0.1     
latest_gesture = "NONE"
# ================== WEBSOCKET SENDER ==================
async def websocket_sender():
    ...
    while True:
        try:
            ...
            async with websockets.connect("ws://YOUR_URL_HERE") as ws:  # placeholder
                print("✅ Connected to ESP8266 WebSocket")

                while True:
                    gesture = latest_gesture.strip()
...
while cap.isOpened():
    ...
    command = {"theta1": 0, "theta2": 0, "gripper": 0}
    ...
    # --- OUTPUT LIST ---
    output = [round(theta1, 2), round(theta2, 2), round(gripper, 2)]
