...
# ================== WEBSOCKET CONFIG ==================
WEBSOCKET_URL = "ws://10.61.66.221:81"
SEND_INTERVAL = 0.1     
...
def detect_gesture(hand_lms, hand_label):
    ...
    # RIGHT HAND → theta1 + base stepper
    if hand_label == "Right":
...
while cap.isOpened():
    ...
    # Default commands
    command = {"theta1": 0, "theta2": 180, "gripper": 0, "base": 0}
    ...
    elif command["theta1"]==2:
        theta1 -=command["theta1"] * STEP 
    else: theta1+=0
    
    if command["theta2"]==1:
        theta2 += command["theta2"] * STEP
    elif command["theta2"]==2:
        theta2-= command["theta2"] * STEP
    else: theta2 +=0
     
    gripper += command["gripper"] * STEP if command["gripper"]==1 else -command["gripper"] * STEP if command["gripper"]==2 else 0
    ...
    cv2.putText(frame, f"Base Dir: {base}", (50, 200),
                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,0), 2)
